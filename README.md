...
Use **Career → Manage roles** when different CV imports describe the same employment period differently. A confirmed merge keeps the selected destination role, fills only its missing metadata, moves all distinct facts, CV sources, and questions, and combines exact duplicates. Facts can retain provenance from multiple CV versions. Roles can also be reordered for whole-career generation or permanently deleted after an explicit `DELETE` confirmation. Merge and deletion never alter the immutable evidence snapshots stored with previously generated application packs.
...
Use **Documents → Create master CV** (or the matching Career action) to build a general-purpose CV without a job advert. This deterministic builder includes every active career fact, preserves each fact's wording, and uses the selected emphasis only to order evidence within a role. It creates a normal reusable master-CV version with Word and PDF downloads, saves the exact career and contact snapshots used, and does not require or charge an AI provider.
