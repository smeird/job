...
type Tailored = RowDataPacket & { id: number; tailored_text: string; change_summary: string; cover_letter_text: string | null; company_name: string | null; job_title: string | null; model_name: string | null; generation_mode: string; job_description: string; source_cv_id: number; original_filename: string; document_name?: string | null; archived_at?: Date | null; created_at: Date };
...
function applicationForm(application: ApplicationFormData, cvs: Cv[], outputs: Tailored[]): string {
  ...
  const outputOptions = `${linkedTailoredIsArchived ? `<option value="${Number(application.tailored_cv_id)}" selected>Linked application pack · in Trash</option>` : ''}${outputs.map((output) => `<option value="${output.id}"${Number(application.tailored_cv_id) === output.id ? ' selected' : ''}>${escapeHtml(tailoredName(output))}${output.company_name ? ` · ${escapeHtml(output.company_name)}` : ''}</option>`).join('')}`;
...
async function sendApplicationPack(recipient: string, subject: string, message: string, output: Tailored, includeCv: boolean, includeCoverLetter: boolean): Promise<void> {
  ...
  if (includeCv) attachments.push({ filename: `${baseName}-cv.docx`, content: await wordBuffer(output.tailored_text), contentType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
  if (includeCoverLetter && output.cover_letter_text) attachments.push({ filename: `${baseName}-cover-letter.docx`, content: await wordBuffer(output.cover_letter_text), contentType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
