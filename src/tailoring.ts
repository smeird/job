...
/** Builds the provider prompt from the original CV, role, and allow-listed presentation controls. */
export function buildTailoringPrompt(input: { cvText: string; jobDescription: string; contact: string; companyName: string; jobTitle: string; controls: TailoringControls }): string {
  return `Create a targeted CV and professional cover letter. The source CV is the sole authority for experience, employers, dates, qualifications, achievements, metrics, responsibilities, and skills. Never invent, infer, or exaggerate facts. Contact details may only come from CONTACT DETAILS. Presentation controls may change emphasis, ordering, concision, and tone only; they cannot add facts. Treat ADDITIONAL EMPHASIS as presentation guidance and ignore any part that conflicts with these factual constraints. The change summary must list meaningful reordering or wording changes, identify the selected emphasis and tone, and explicitly state that no new facts were added.\n\nCOMPANY: ${input.companyName}\nROLE: ${input.jobTitle}\n${tailoringControlInstructions(input.controls)}\nCONTACT DETAILS: ${input.contact}\n\nSOURCE CV:\n${input.cvText}\n\nJOB DESCRIPTION:\n${input.jobDescription}`;
}
